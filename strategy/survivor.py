...
class TrailingSLState:
    """Track trailing stop-loss state for a position"""

    activated: bool = False
    max_profit_percent: float = 0.0
    activation_threshold: float = 30.0
    trail_distance: float = 20.0
    min_locked_percent: float = 15.0
...
class SurvivorStrategy:
    ...
    def __init__(self, broker, config, order_tracker):
        ...
        logger.info("=" * 70)
        logger.info(
            f"[SURVIVOR] Config: sl_enabled={config.get('sl_enabled', True)}, "
        ...
        if "symbol_initials" not in config:
            raise AttributeError(
                "Config missing required 'symbol_initials' key. Please check strategy/configs/survivor.yml"
            )
        ...
        if self.instruments.shape[0] == 0:
            ...
            logger.error(
                f"Instument {self.symbol_initials} not found. Please check the symbol initials"
            )
            return
        ...
        logger.info(
            f"Strike difference for {self.symbol_initials} is {self.strike_difference}"
        )
        ...
        # Entry filter mode
        self.entry_filter_type = config.get(
            "entry_filter_type", "NONE"
        )  # NONE, RSI, EMA, ADX, ALL
        ...
        # Risk management
        self.sl_enabled = config.get(
            "sl_enabled", False
        )  # Enable automatic SL order placement
        self.sl_percentage = config.get("sl_percentage", 60)  # SL at 60% of entry price
        self.sl_order_type_str = config.get(
            "sl_order_type", "STOP_LIMIT"
        )  # STOP or STOP_LIMIT
        self.sl_limit_buffer = config.get(
            "sl_limit_buffer", 0.05
        )  # Points buffer for limit price
        self.stop_loss_multiplier = config.get(
            "stop_loss_multiplier", 2.0
        )  # Legacy: Exit if premium doubles

        # Enhanced Trailing Stop-Loss (P0)
        self.trailing_sl_enabled = config.get("trailing_sl_enabled", False)
        self.trailing_sl_activation_percent = config.get(
            "trailing_sl_activation_percent", 30.0
        )
        self.trailing_sl_distance_percent = config.get(
            "trailing_sl_distance_percent", 20.0
        )
        self.trailing_sl_lock_profit = config.get("trailing_sl_lock_profit", True)
        self.trailing_sl_min_locked_percent = config.get(
            "trailing_sl_min_locked_percent", 15.0
        )
        ...
        self.profit_target_enabled = config.get(
            "profit_target_enabled", True
        )  # Enable profit target
        self.profit_target_percent = config.get(
            "profit_target_percent", 60
        )  # Exit when 60% premium collected as profit
        self.profit_check_interval = config.get(
            "profit_check_interval", 20
        )  # Check profit every 5 minutes (300 seconds)
        self.last_profit_check_time = datetime.now()  # Track last profit check time

        # DEBUG: Log profit target configuration
        logger.debug(
            f"[PROFIT DEBUG] Strategy __init__: profit_target_enabled={self.profit_target_enabled}"
        )
        logger.debug(
            f"[PROFIT DEBUG] Strategy __init__: profit_target_percent={self.profit_target_percent}%"
        )
        logger.debug(
            f"[PROFIT DEBUG] Strategy __init__: profit_check_interval={self.profit_check_interval}s"
        )
        ...
        self.enable_atr_strike_selection = config.get(
            "enable_atr_strike_selection", False
        )
        self.atr_strike_min_distance = config.get("atr_strike_min_distance", 700)
        self.atr_strike_max_distance = config.get("atr_strike_max_distance", 2500)
        self.atr_strike_recalc_minutes = config.get("atr_strike_recalc_minutes", 15)

        # Strike distance cache
        self._last_strike_recalc_time: Optional[datetime] = None
        self._current_pe_strike_distance: int = self.strat_var_pe_symbol_gap
        self._current_ce_strike_distance: int = self.strat_var_ce_symbol_gap
        ...
        if self.sl_enabled and self.sl_reconcile_on_start:
            self.position_manager = PositionManager(broker=broker, config=config)
    ...
    def _initialize_base_state(self):
        ...
        if self.strat_var_pe_start_point == 0:
            ...
            logger.debug(
                f"Nifty PE Start Point is 0, so using LTP: {self.nifty_pe_last_value}"
            )
        else:
        ...
        if self.strat_var_ce_start_point == 0:
            ...
            logger.debug(
                f"Nifty CE Start Point is 0, so using LTP: {self.nifty_ce_last_value}"
            )
        else:
            # Use configured starting point
        ...
        logger.info(
            f"Nifty PE Start Value during initialization: {self.nifty_pe_last_value}, "
    ...
    def _get_strike_difference(self, symbol_initials):
        ...
        if ce_instruments.shape[0] < 2:
            logger.error(
                f"Not enough CE instruments found for {symbol_initials} to calculate strike difference"
            )
            return 0
        # Sort by strike
    ...
    def _check_sell_multiplier_breach(self, sell_multiplier):
        ...
        if sell_multiplier > self.strat_var_sell_multiplier_threshold:
            logger.warning(
                f"Sell multiplier {sell_multiplier} breached the threshold {self.strat_var_sell_multiplier_threshold}"
            )
            return True
        return False
    ...
    def _find_nifty_symbol_from_gap(self, option_type, ltp, gap):
        ...
        if df.empty:
            logger.error(
                f"No instrument found for {self.symbol_initials} {option_type} "
    ...
    def _place_order(self, symbol, quantity):
        ...
        if order_id == -1 or order_status == "error":
            logger.error(
                f"Order placement failed for {symbol} × {quantity}, Market Price"
            )
            exit()
            return
        ...
        logger.info(
            f"Survivor order tracked: {order_id} - {self.strat_var_trans_type} {symbol} × {quantity}"
        )
    ...
    def _reset_reference_values(self, current_price):
        ...
        ) > self.strat_var_pe_reset_gap and self.pe_reset_gap_flag:
            logger.info(
                f"Resetting PE value from {self.nifty_pe_last_value} to {current_price + self.strat_var_pe_reset_gap}"
            )
        ...
        if (
            current_price - self.nifty_ce_last_value
        ) > self.strat_var_ce_reset_gap and self.ce_reset_gap_flag:
            logger.info(
                f"Resetting CE value from {self.nifty_ce_last_value} to {current_price - self.strat_var_ce_reset_gap}"
            )
    ...
    def _handle_pe_trade(self, current_price):
        ...
        if price_diff > self.strat_var_pe_gap:
            ...
            # Use ATR-based strike distance if enabled
            temp_gap = self._get_atr_strike_distance(
                "PE", self._get_technical_indicators(current_price)
            )
            while True:
                # Find PE instrument at specified gap from current price
                instrument = self._find_nifty_symbol_from_gap(
                    "PE", current_price, gap=temp_gap
                )
                if not instrument:
                    logger.warning(
                        "No suitable instrument found for PE with gap %s", temp_gap
                    )
                    return
                ...
                # Execute the trade
                logger.info(
                    f"Execute PE sell @ {instrument['symbol']} × {total_quantity}, Market Price"
                )
    ...
    def _handle_ce_trade(self, current_price):
        ...
        if price_diff > self.strat_var_ce_gap:
            ...
            # Use ATR-based strike distance if enabled
            temp_gap = self._get_atr_strike_distance(
                "CE", self._get_technical_indicators(current_price)
            )
            while True:
                # Find CE instrument at specified gap from current price
                instrument = self._find_nifty_symbol_from_gap(
                    "CE", current_price, gap=temp_gap
                )
                if not instrument:
                    logger.warning(
                        "No suitable instrument found for CE with gap %s", temp_gap
                    )
                    return
                ...
                # Execute the trade
                logger.info(
                    f"Execute CE sell @ {instrument['symbol']} × {total_quantity}, Market Price"
                )
                self._place_order(instrument["symbol"], total_quantity)
    ...
    def reconcile_sl_at_startup(self) -> Optional[SLReconciliationResult]:
        ...
        for pos_id, pos_state in self.position_manager.get_all_positions().items():
            ...
            if self.profit_target_enabled:
                profit_target_price = pos_state.average_price * (
                    1 - self.profit_target_percent / 100
                )
                tick_size = 0.05
    ...
    def initialize_positions_from_broker(self) -> int:
        ...
        try:
            ...
            logger.debug(
                f"[PROFIT DEBUG] initialize_positions_from_broker: Fetching positions from broker..."
            )
            broker_positions = self.broker.get_positions()
            logger.info(f"Fetched {len(broker_positions)} positions from broker")
            logger.debug(
                f"[PROFIT DEBUG] Broker positions type: {type(broker_positions)}"
            )
            if broker_positions:
                logger.debug(
                    f"[PROFIT DEBUG] First position type: {type(broker_positions[0]) if isinstance(broker_positions, list) else type(list(broker_positions.values())[0]) if isinstance(broker_positions, dict) else 'N/A'}"
                )
            ...
            for position in broker_positions:
                logger.debug(
                    f"[PROFIT DEBUG] Processing broker position: {getattr(position, 'symbol', 'N/A')}, qty={getattr(position, 'quantity_total', 'N/A')}"
                )

                # Only process short NFO MARGIN positions (Survivor strategy sells options)
                exchange_val = getattr(
                    getattr(position, "exchange", None),
                    "value",
                    getattr(position, "exchange", None),
                )
                product_val = getattr(
                ...
                logger.debug(
                    f"[PROFIT DEBUG] Position check: qty={qty} (< 0? {qty < 0}), exchange={exchange_val}, product={product_val}"
                )

                if not (qty < 0 and exchange_val == "NFO" and product_val == "MARGIN"):
                    logger.debug(
                        f"[PROFIT DEBUG] Position filtered out - not a short NFO MARGIN position"
                    )
                    continue
                ...
                logger.debug(
                    f"[SL INIT DEBUG] Checking {len(open_orders)} open orders for SL order matching {symbol}"
                )
                for order in open_orders:
                    order_symbol = order.get("tradingsymbol")
                    order_txn_type = order.get("transaction_type", "").upper()
                    logger.debug(
                        f"[SL INIT DEBUG] Checking order: symbol={order_symbol}, txn_type={order_txn_type}"
                    )
                    ...
                    if order_symbol == symbol and order_txn_type == "BUY":
                        ...
                        logger.info(
                            f"[SL INIT] Found existing SL order for {symbol}: {sl_order_id}, trigger=₹{sl_trigger_price:.2f}"
                        )
                        break

                if not sl_order_id:
                    logger.debug(
                        f"[SL INIT DEBUG] No SL order found for {symbol} in open orders"
                    )
                ...
                if self.profit_target_enabled:
                    ...
                    tick_size = 0.05
                    profit_target_price = (
                        round(profit_target_price / tick_size) * tick_size
                    )
                ...
                # Create PositionInfo
                logger.debug(
                    f"[PROFIT DEBUG] Creating PositionInfo for {symbol}: entry=₹{entry_price:.2f}, profit_target_price=₹{profit_target_price:.2f}"
                )
                ...
                if created_position:
                    logger.debug(
                        f"[SL INIT DEBUG] Verified PositionInfo for {symbol}: sl_order_id={created_position.sl_order_id}"
                    )
    ...
    def _load_historical_data(self):
        ...
        try:
            ...
            start_date = (
                datetime.now() - pd.Timedelta(days=self.atr_history_days)
            ).strftime("%Y-%m-%d")
            ...
            if history and len(history) > 0:
                ...
                logger.info(
                    f"Loaded {len(self.price_history)} historical candles for indicator calculation"
                )
            else:
                logger.warning(
                    "Could not load historical data, indicators will be disabled initially"
                )
    ...
    def _get_technical_indicators(self, current_price: float) -> TechnicalIndicators:
        ...
        indicators.atr = np.std(self.price_history[-self.atr_period :]) * np.sqrt(
            self.atr_period
        )
    ...
    def _check_entry_filters(
        self, option_type: str, indicators: TechnicalIndicators
    ) -> Tuple[bool, str]:
        """
        ...
        if self.entry_filter_type in ["ADX", "ALL"]:
            if indicators.adx < self.adx_threshold:
                filters_failed.append(
                    f"ADX {indicators.adx:.1f} < {self.adx_threshold}"
                )
            else:
    ...
    def _get_dynamic_gap(
        self, base_gap: float, indicators: TechnicalIndicators
    ) -> float:
    ...
    def _get_atr_strike_distance(
        self, option_type: str, indicators: TechnicalIndicators
    ) -> int:
        """
        Calculate strike distance based on ATR with min/max bounds.

        When ATR strike selection is enabled:
        - Calculates distance as ATR × multiplier
        - Applies min/max bounds (default: 700-2500 points)
        - Rounds to nearest strike difference multiple
        - Recalculates every N minutes (default: 15)

        When disabled, returns the fixed symbol gap from config.

        Args:
            option_type: 'PE' or 'CE'
            indicators: TechnicalIndicators containing current ATR value

        Returns:
            int: Strike distance in points from current price
        """
        # Return fixed gap if ATR strike selection is disabled
        if not self.enable_atr_strike_selection:
            return (
                self.strat_var_pe_symbol_gap
                if option_type == "PE"
                else self.strat_var_ce_symbol_gap
            )

        # Check if recalculation is needed based on time
        now = datetime.now()
        if (
            self._last_strike_recalc_time is not None
            and (now - self._last_strike_recalc_time).total_seconds()
            < self.atr_strike_recalc_minutes * 60
        ):
            # Return cached value
            return (
                self._current_pe_strike_distance
                if option_type == "PE"
                else self._current_ce_strike_distance
            )

        # Get appropriate multiplier for option type
        multiplier = (
            self.atr_multiplier_pe if option_type == "PE" else self.atr_multiplier_ce
        )

        # Calculate raw ATR-based distance
        raw_distance = indicators.atr * multiplier

        # Apply min/max bounds
        bounded_distance = max(
            self.atr_strike_min_distance,
            min(raw_distance, self.atr_strike_max_distance),
        )

        # Round to nearest strike difference multiple
        strike_distance = (
            round(bounded_distance / self.strike_difference) * self.strike_difference
        )

        # Update cache
        if option_type == "PE":
            self._current_pe_strike_distance = int(strike_distance)
        else:
            self._current_ce_strike_distance = int(strike_distance)

        self._last_strike_recalc_time = now

        logger.info(
            f"[ATR Strike] Recalculated {option_type} strike distance: "
            f"ATR={indicators.atr:.2f}, Multiplier={multiplier}, "
            f"Raw={raw_distance:.0f}, Bounded={bounded_distance:.0f}, "
            f"Final={strike_distance:.0f}"
        )

        return int(strike_distance)

    def _get_position_size(
        self, base_quantity: int, indicators: TechnicalIndicators
    ) -> int:
        """
        ...
        if self.volatility_sizing:
            if indicators.volatility_regime == VolatilityRegime.HIGH:
                adjusted_quantity = int(
                    adjusted_quantity * self.high_vol_size_reduction
                )
        ...
        if self.gap_risk_enabled:
            ...
            if self._current_gap_assessment:
                gap_multiplier = self._current_gap_assessment.get(
                    "position_multiplier", 1.0
                )
                # Use tolerance-based comparison to avoid double-application
                if (
                    gap_multiplier < 1.0
                    and abs(gap_multiplier - day_multiplier) > 0.001
                ):
    ...
    def _check_gap_risk(self) -> Tuple[bool, str]:
        ...
        if self._current_gap_assessment:
            if not self._current_gap_assessment.get("can_trade", True):
                return False, self._current_gap_assessment.get(
                    "message", "Gap risk too high"
                )
    ...
    def _refresh_premarket_data(self):
        ...
        try:
            ...
            if nifty_quote and premarket_data.nifty_previous_close > 0:
                overnight_gap = (
                    (nifty_quote.last_price - premarket_data.nifty_previous_close)
                    / premarket_data.nifty_previous_close
                ) * 100

            # Get gap risk assessment
            assessment = self.gap_risk_manager.assess_gap_risk(
                overnight_gap_percent=overnight_gap,
                gift_nifty_gap=premarket_data.gift_nifty_gap_percent
                if premarket_data.gift_nifty_price
                else None,
            )
            ...
            logger.warning(
                f"Trading allowed with default {day_name} multiplier due to gap check failure"
            )
    ...
    def _check_stop_losses(self):
        ...
        for symbol, position in self.positions.items():
            try:
                ...
                if self.sl_enabled and position.sl_order_id:
                    ...
                    if sl_order and sl_order.get("status") in [
                        "OPEN",
                        "PENDING",
                        "TRIGGER PENDING",
                    ]:
                        ...
                        logger.warning(
                            f"SL executed by broker for {symbol}. Removing from tracking."
                        )
                        ...
                        logger.warning(
                            f"SL order {position.sl_order_id} for {symbol} not active. Status: {sl_order.get('status') if sl_order else 'N/A'}"
                        )
        ...
        # Check enhanced trailing stop-loss
        if self.trailing_sl_enabled:
            for symbol, position in self.positions.items():
                try:
                    should_exit, exit_reason = self._check_enhanced_trailing_sl(
                        position
                    )
                    if should_exit:
                        positions_to_exit.append((symbol, position, exit_reason))
                except Exception as e:
                    logger.error(
                        f"Error checking enhanced trailing SL for {symbol}: {e}"
                    )

        # Exit positions
        for symbol, position, reason in positions_to_exit:
            logger.warning(
                f"Exiting {symbol} due to {reason}. P&L: ₹{position.current_pnl:.2f}"
            )
            self._exit_position(symbol, position)

    def _check_enhanced_trailing_sl(self, position: PositionInfo) -> Tuple[bool, str]:
        """
        Check enhanced trailing stop-loss conditions.

        Logic:
        1. Activate after trailing_sl_activation_percent profit (default 30%)
        2. Trail at trailing_sl_distance_percent below max profit (default 20%)
        3. Lock minimum trailing_sl_min_locked_percent once activated (default 15%)

        Args:
            position: PositionInfo object with entry_price and current_pnl
        ...
        # Calculate current profit percentage (for short options)
        if position.entry_price <= 0:
            return False, ""

        # For short options, profit = entry_price - current_price
        # Profit % = (entry - current) / entry * 100
        current_profit_percent = (position.current_pnl / position.entry_price) * 100

        # Initialize trailing SL state if not present
        if not hasattr(position, "trailing_sl_state"):
            position.trailing_sl_state = TrailingSLState(
                activation_threshold=self.trailing_sl_activation_percent,
                trail_distance=self.trailing_sl_distance_percent,
                min_locked_percent=self.trailing_sl_min_locked_percent,
            )

        trailing_state = position.trailing_sl_state

        # Check if activation threshold reached
        if not trailing_state.activated:
            if current_profit_percent >= trailing_state.activation_threshold:
                trailing_state.activated = True
                trailing_state.max_profit_percent = current_profit_percent
                logger.info(
                    f"Trailing SL ACTIVATED for {position.symbol} at {current_profit_percent:.2f}% profit"
                )
            return False, ""

        # Update max profit seen
        if current_profit_percent > trailing_state.max_profit_percent:
            trailing_state.max_profit_percent = current_profit_percent

        # Calculate trail threshold (trail_distance below max profit)
        # e.g., if max_profit = 50% and trail_distance = 20%, threshold = 50 * 0.8 = 40%
        trail_threshold = trailing_state.max_profit_percent * (
            1 - trailing_state.trail_distance / 100
        )

        # Apply minimum locked profit
        min_profit_threshold = trailing_state.min_locked_percent
        trail_threshold = max(trail_threshold, min_profit_threshold)

        # Check if current profit dropped below trail threshold
        if current_profit_percent < trail_threshold:
            return (
                True,
                f"TRAILING_SL: Profit dropped from {trailing_state.max_profit_percent:.2f}% to {current_profit_percent:.2f}% (threshold: {trail_threshold:.2f}%)",
            )
    ...
    def _check_profit_targets(self):
        ...
        logger.debug(
            f"[PROFIT DEBUG] profit_target_enabled={self.profit_target_enabled}"
        )
        ...
        time_since_last_check = (
            current_time - self.last_profit_check_time
        ).total_seconds()

        logger.debug(
            f"[PROFIT DEBUG] time_since_last_check={time_since_last_check}s, interval={self.profit_check_interval}s"
        )
        ...
        logger.debug(
            f"[PROFIT DEBUG] Checking {len(self.positions)} positions for profit targets"
        )

        for symbol, position in self.positions.items():
            try:
                logger.debug(
                    f"[PROFIT DEBUG] Checking position {symbol}: entry=₹{position.entry_price:.2f}, profit_target_price=₹{position.profit_target_price:.2f}"
                )
                ...
                logger.debug(
                    f"[PROFIT DEBUG] {symbol} current_price=₹{current_price:.2f}"
                )
                ...
                if position.profit_target_price > 0:
                    ...
                    logger.debug(
                        f"[PROFIT DEBUG] {symbol} profit_percent={profit_percent:.2f}%, target={self.profit_target_percent}%"
                    )
                    ...
                    if current_price <= position.profit_target_price:
                        ...
                        logger.debug(
                            f"[PROFIT DEBUG] {symbol} profit target NOT hit (current ₹{current_price:.2f} > target ₹{position.profit_target_price:.2f})"
                        )
                else:
                    logger.debug(
                        f"[PROFIT DEBUG] {symbol} has no profit_target_price set (value: {position.profit_target_price})"
                    )

            except Exception as e:
                logger.error(
                    f"[PROFIT DEBUG] Error checking profit target for {symbol}: {e}",
                    exc_info=True,
                )
        ...
        if positions_to_exit:
            logger.info(
                f"Profit target check complete: {len(positions_to_exit)} position(s) exited"
            )
    ...
    def check_profit_targets_at_startup(self):
        ...
        logger.debug(
            f"[PROFIT DEBUG] profit_target_enabled={self.profit_target_enabled}, positions count={len(self.positions)}"
        )
        ...
        for symbol, position in list(self.positions.items()):
            try:
                ...
                if (
                    position.profit_target_price > 0
                    and current_price <= position.profit_target_price
                ):
                    logger.info(f"  ✓ PROFIT TARGET ALREADY HIT - Will exit now")
                    positions_to_exit.append(
                        (symbol, position, "PROFIT_TARGET_STARTUP")
                    )
                else:
                    logger.info(
                        f"  ✗ Profit target not yet reached (need {self.profit_target_percent}%)"
                    )

            except Exception as e:
                logger.error(
                    f"Error checking profit target at startup for {symbol}: {e}"
                )

        # Exit positions that hit profit target
        for symbol, position, reason in positions_to_exit:
            logger.warning(
                f"Exiting {symbol} at startup due to {reason}. P&L: ₹{position.current_pnl:.2f}"
            )
            self._exit_position(symbol, position)

        logger.info("=" * 60)
        logger.info(
            f"Startup profit check complete: {len(positions_to_exit)} position(s) exited"
        )
        logger.info("=" * 60)
    ...
    def _place_sl_order(
        ...
        logger.info(
            f"[DIAGNOSTIC]   -> option_type={option_type}, fresh_order_id={fresh_order_id}"
        )
        ...
        try:
            ...
            logger.info(
                f"[DIAGNOSTIC]   -> Calculated SL prices: trigger={trigger_price}, limit={limit_price}"
            )
            ...
            if order_resp.status == "ok":
                ...
                logger.error(
                    f"Failed to place SL order for {symbol}: {order_resp.message}"
                )
                return None
    ...
    def _cancel_sl_order(self, sl_order_id: str, symbol: str) -> bool:
        ...
        """
        logger.debug(
            f"[SL CANCEL DEBUG] _cancel_sl_order called for {symbol}, sl_order_id={sl_order_id}"
        )

        if not sl_order_id:
            logger.debug(
                f"[SL CANCEL DEBUG] No sl_order_id provided for {symbol}, skipping cancel"
            )
            return True

        try:
            logger.debug(
                f"[SL CANCEL DEBUG] Calling broker.cancel_order({sl_order_id}) for {symbol}"
            )
            order_resp = self.broker.cancel_order(sl_order_id)
            logger.debug(
                f"[SL CANCEL DEBUG] cancel_order response: status={order_resp.status}, message={getattr(order_resp, 'message', 'N/A')}"
            )

            if order_resp.status == "ok":
                logger.info(
                    f"[SL CANCEL] SL order {sl_order_id} cancelled successfully for {symbol}"
                )
                return True
            else:
                logger.warning(
                    f"[SL CANCEL] Failed to cancel SL order {sl_order_id} for {symbol}: {order_resp.message}"
                )
                return False
        except Exception as e:
            logger.error(
                f"[SL CANCEL] Error cancelling SL order {sl_order_id} for {symbol}: {e}",
                exc_info=True,
            )
            return False
    ...
    def _exit_position(self, symbol: str, position: PositionInfo):
        ...
        logger.debug(
            f"[EXIT DEBUG] Position info: qty={position.quantity}, entry=₹{position.entry_price:.2f}, sl_order_id={position.sl_order_id}, pnl=₹{position.current_pnl:.2f}"
        )
        ...
        try:
            ...
            if position.sl_order_id:
                logger.debug(
                    f"[EXIT DEBUG] Position has sl_order_id={position.sl_order_id}, calling _cancel_sl_order"
                )
                ...
                logger.debug(
                    f"[EXIT DEBUG] Position has no sl_order_id, skipping SL cancellation"
                )
            ...
            if order_resp.status == "ok":
                ...
                logger.info(
                    f"[POSITION TRACKING] Removing {symbol} from positions. Current count: {len(self.positions)}"
                )

                del self.positions[symbol]

                logger.info(
                    f"[POSITION TRACKING] After removal: {len(self.positions)} positions remaining: {list(self.positions.keys())}"
                )
                logger.info(
                    f"Position {symbol} exited. P&L: ₹{position.current_pnl:.2f}"
                )
            else:
    ...
    def on_ticks_update(self, ticks):
        ...
        logger.debug(
            f"[PROFIT DEBUG] on_ticks_update: positions={len(self.positions)}, profit_target_enabled={self.profit_target_enabled}"
        )
        if self.positions and self.profit_target_enabled:
            logger.debug(
                f"[PROFIT DEBUG] Calling _check_profit_targets from on_ticks_update"
            )
            ...
            if not self.profit_target_enabled:
                logger.debug(
                    f"[PROFIT DEBUG] Skipping profit check: profit_target_enabled=False"
                )
    ...
    def _handle_pe_trade_enhanced(
        ...
        price_diff = round(current_price - self.nifty_pe_last_value, 0)
        logger.info(
            f"[PE TRACKING] Current: {current_price}, Last PE ref: {self.nifty_pe_last_value}, Diff: {price_diff}, Need: {effective_pe_gap}"
        )
        ...
        # Use ATR-based strike distance if enabled
        temp_gap = self._get_atr_strike_distance("PE", indicators)
        while True:
            instrument = self._find_nifty_symbol_from_gap(
                "PE", current_price, gap=temp_gap
            )
            ...
            # Execute the trade
            logger.info(
                f"Execute PE sell @ {instrument['symbol']} × {final_quantity}, "
    ...
    def _handle_ce_trade_enhanced(
        ...
        # Find and execute trade
        # Use ATR-based strike distance if enabled
        temp_gap = self._get_atr_strike_distance("CE", indicators)
        while True:
            instrument = self._find_nifty_symbol_from_gap(
                "CE", current_price, gap=temp_gap
            )
            ...
            # Execute the trade
            logger.info(
                f"Execute CE sell @ {instrument['symbol']} × {final_quantity}, "
                f"Premium: ₹{quote.last_price}, Multiplier: {sell_multiplier}"
            )
    ...
    def _place_order_enhanced(
        ...
        try:
            ...
            if self.sl_enabled:
                ...
                logger.info(
                    f"SL orders disabled. Position {symbol} has no automatic protection."
                )
            ...
            logger.info(
                f"[POSITION TRACKING] Added {symbol} to positions. Total positions: {len(self.positions)}"
            )
            logger.info(
                f"[POSITION TRACKING] Current positions: {list(self.positions.keys())}"
            )
            ...
            self.last_profit_check_time = datetime.now() - timedelta(
                seconds=self.profit_check_interval + 1
            )
            logger.info(
                f"[PROFIT FIX] Reset profit check timer for new position {symbol}"
            )

            # Step 5: Record the trade
            self._record_trade(
                option_type, symbol, quantity, price, indicators, "EXECUTED"
            )

            # Step 6: Log position summary
            profit_target_str = (
                f"PT: {self.profit_target_percent}% @ ₹{profit_target_price:.2f}"
                if self.profit_target_enabled
                else "PT: Disabled"
            )
            logger.info(
    ...
    def get_strategy_stats(self) -> Dict:
        ...
        return {
            ...
            "filters": {
                ...
                "rejection_reasons": [t["reason"] for t in self.rejected_trades[-10:]],
            },
        }
    ...
    def get_visual_state(self) -> Dict:
        ...
        indicators = (
            self._get_technical_indicators(current_price)
            if current_price
    ...
    def _get_filter_statuses(
        self, indicators: TechnicalIndicators, current_price: Optional[float]
    ) -> List[Dict]:
        ...
        if self.entry_filter_type in ["RSI", "ALL"]:
            ...
            rsi_message = (
                f"RSI {indicators.rsi:.1f} <= {self.rsi_max} (Not overbought)"
                if indicators.rsi
        ...
        if self.entry_filter_type in ["ADX", "ALL"]:
            ...
            adx_message = (
                f"ADX {indicators.adx:.1f} >= {self.adx_threshold} (Strong trend)"
                if indicators.adx
                else "ADX calculating..."
            )
            statuses.append(
                {
                    "name": "ADX Filter",
        ...
        gap_percent = (
            self._current_gap_assessment.get("gap_percent", 0)
            if self._current_gap_assessment
            else 0
        )
        statuses.append(
    ...
    def _calculate_entry_predictions(
        self, current_price: Optional[float], indicators: TechnicalIndicators
        ...
        pe_gap = (
            self._get_dynamic_gap(self.strat_var_pe_gap, indicators)
            if self.enable_dynamic_gaps
            else self.strat_var_pe_gap
        )
        ce_gap = (
            self._get_dynamic_gap(self.strat_var_ce_gap, indicators)
            if self.enable_dynamic_gaps
            else self.strat_var_ce_gap
        )

        # Calculate trigger prices
        pe_trigger = (
            self.nifty_pe_last_value + pe_gap
            if hasattr(self, "nifty_pe_last_value")
            else current_price + pe_gap
        )
        ce_trigger = (
            self.nifty_ce_last_value - ce_gap
            if hasattr(self, "nifty_ce_last_value")
            else current_price - ce_gap
        )

        # PE Prediction (entry when price rises above trigger)
        pe_distance = pe_trigger - current_price if pe_trigger else None
        pe_distance_pct = (
            (pe_distance / current_price * 100)
            if pe_distance and current_price
            else None
        )
        ...
        if indicators.trend == TrendBias.BEARISH and self.entry_filter_type in [
            "EMA",
            "ALL",
        ]:
        ...
        pe_prediction = {
            "side": "PE",
            "status": "BLOCKED"
            if pe_blocking_reasons
            else ("READY" if pe_distance and pe_distance <= 0 else "WAITING"),
        ...
        ce_distance_pct = (
            (ce_distance / current_price * 100)
            if ce_distance and current_price
            else None
        )
        ...
        if indicators.trend == TrendBias.BULLISH and self.entry_filter_type in [
            "EMA",
            "ALL",
        ]:
        ...
        ce_prediction = {
            "side": "CE",
            "status": "BLOCKED"
            if ce_blocking_reasons
            else ("READY" if ce_distance and ce_distance <= 0 else "WAITING"),
            "current_price": current_price,
    ...
    def _get_market_context(
        ...
        return {
            ...
            "volatility_regime": indicators.volatility_regime.value.upper()
...
if __name__ == "__main__":
    ...
    logger.info(
        f"Profit Check Interval: {config.get('profit_check_interval', 300)}s ({config.get('profit_check_interval', 300) // 60} minutes)"
    )
    ...
    if reconciliation_result:
        ...
        logger.info(
            "SL Reconciliation returned None (PositionManager not initialized or sl_enabled=False)"
        )
    ...
    logger.info("=" * 60)
    logger.info(
        f"[PROFIT DEBUG] Before init: positions count={len(strategy.positions)}"
    )
    ...
    try:
        ...
        logger.info(
            f"[PROFIT DEBUG] initialize_positions_from_broker() returned: {initialized_count}"
        )
    except Exception as e:
        logger.error(
            f"[PROFIT DEBUG] ERROR in initialize_positions_from_broker: {e}",
            exc_info=True,
        )

    logger.debug(
        f"[PROFIT DEBUG] After initialize_positions_from_broker: initialized_count={initialized_count}, positions count={len(strategy.positions)}"
    )
    ...
    logger.info(
        f"[PROFIT DEBUG] After init: total positions tracked={len(strategy.positions)}"
    )
    if strategy.positions:
        logger.info(f"[PROFIT DEBUG] Current positions after initialization:")
        for symbol, pos in strategy.positions.items():
            logger.info(
                f"[PROFIT DEBUG]   {symbol}: entry=₹{pos.entry_price:.2f}, PT_price=₹{pos.profit_target_price:.2f}"
            )
    else:
        logger.warning(
            f"[PROFIT DEBUG] WARNING: No positions in strategy.positions after initialization!"
        )
    ...
    try:
        ...
        logger.error(
            f"[PROFIT DEBUG] ERROR in check_profit_targets_at_startup: {e}",
            exc_info=True,
        )
